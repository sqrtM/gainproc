...
int inci(s_Mix *mix, s_Small_Register m, unsigned int i) {
    ...
    // Overflow is intentionally not checked.
...
int deca(s_Mix *mix, s_Small_Register m) {
    ...
    if (mix->A->sign != w.sign && mix->A->value > WORD_MAX - w.value) {
        mix->overflow = true;
    }

    *mix->A = sub_words(*mix->A, w);
    return 0;
}
...
int decx(s_Mix *mix, s_Small_Register m) {
    ...
    if (mix->X->sign != w.sign && mix->X->value > WORD_MAX - w.value) {
        mix->overflow = true;
    }

    *mix->X = sub_words(*mix->X, w);
    return 0;
}
